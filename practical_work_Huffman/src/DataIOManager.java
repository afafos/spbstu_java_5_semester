...
public class DataIOManager {
    static String NAME = "Afanasyev";

    static private void readArchiveHeader(String filePath, CompressionInfo compressionInfo) {
        ...
        compressionInfo.dataOffset = headerData.toString().getBytes().length;
        compressionInfo.recoveryMap = recoveryMap;
        compressionInfo.originalFileName = originalFileName;
    }
    ...
    static private void readArchiveData(String filePath, CompressionInfo compressionInfo) {
        ...
        try {
            ...
            fileInputStream.skip(compressionInfo.dataOffset);
        ...
        compressionInfo.originalLength = originalLength;
        compressionInfo.compressedData = compressed.substring(8, compressed.length() - additionalZerosNumber);
    }

    static public CompressionInfo readArchive(String filePath) {
        CompressionInfo compressionInfo = new CompressionInfo();

        readArchiveHeader(filePath, compressionInfo);
        readArchiveData(filePath, compressionInfo);
    ...
    static public void test(CompressionInfo compressionInfo, byte[] result) {
        System.out.println("Archive length: " + compressionInfo.originalLength);
        System.out.println("Original file name: " + compressionInfo.originalFileName);
        System.out.println("Original file length: " + result.length);

        if (result.length > 0) {
            System.out.println("Compress ratio: " + (100 * compressionInfo.originalLength / result.length) + "%");
        }

        System.out.println("Code map:");

        for (Map.Entry<String, Byte> entry : compressionInfo.recoveryMap.entrySet()) {
